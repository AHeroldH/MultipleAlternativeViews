...
data = pd.read_csv('Experiments/Time/leader_time_2.csv')
...
data['complexity'] = [(float(row['avg_leader_time'])*1000000)/(row['number_of_samples'])for i, row in data.iterrows()]
data['complexity2'] = [(row['number_of_samples']) for i, row in data.iterrows()]
sns.set()
ax = plt.gca()
data.plot(kind='scatter', x='complexity2', y='complexity', ax=ax, color='green')
plt.ylabel('Time in microseconds / number of samples')
plt.xlabel('Number of samples')
ax.axhline(y=4000, color='blue', label='$c$')
ax.axvline(x=6500, color='black', label='$n_0$', linestyle='dashed')
...
plt.savefig('leader_clustering_avg_complexity.png')
plt.show()
